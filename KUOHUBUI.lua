--// KUOHUB FULL ADVANCED + FUTURISTIC + MINIMIZE FIX + RESPONSIVE FINAL    
...
-- 🔥 GLOBAL SCALE (ใช้ตัวเดียวพอ)    
local UIScale = Instance.new("UIScale", Main)    
    
-- =========================    
-- 📱 ADVANCED RESPONSIVE UI (FIXED 100%)    
-- =========================    
...
local function applyResponsiveUI()    
    local size = Camera.ViewportSize    
    local width = size.X    
    local height = size.Y    
    
    local is_phone = isMobile()    
    
    -- 🔥 scale กลาง    
    local scale = math.clamp(width / 1600, 0.75, 1.25)    
    UIScale.Scale = scale    
...
function Window:AddMinimizeButton(config)    
    ...
    Instance.new("UICorner", btn).CornerRadius = config.Corner.CornerRadius or UDim.new(1,0)    
