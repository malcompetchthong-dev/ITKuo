...
local WorldDropdown = AutoWinSection:AddDropdown({
    ...
    Callback = function(Value)
        ...
        -- หาก UI library ไม่มี Refresh ให้เปลี่ยนเป็นการสร้าง Dropdown ใหม่ หรือใช้ SetOptions
...
AutoWinSection:Toggle({
    Title = "Auto Win",
    Desc = "ฟาร์มวินอัตโนมัติ",
    Callback = function(Value)
        Auto_win = Value
        if Value then
            if AutoWin.Start then
                ...
                warn("AutoWin Core ยังไม่โหลดเสร็จ")
                Auto_win = false
            end
        else
            if AutoWin.Stop then
                AutoWin.Stop()
                if Window and Window.Notify then
